---
title: "Batch downloading FASTQ files using the SRA toolkit, fastq-dump, and Python"
author: Erick Lu
...
How do we access the data? Raw sequencing files, even when in their compressed form (usually as .fastq.gz files), are huge files that are often multiple gigabytes in size per sample. If you are a researcher with little bioinformatics experience, the downloading process can be somewhat complicated. This guide explains how to:

1. Navigate through GEO to find sequencing data.
2. Download raw FASTQ files using the NCBI SRA toolkit.
3. Use a Python script to batch download files with prefetch and fastq-dump.

## Finding raw sequencing data in GEO

Let's say you are reading a paper in a journal and see an interesting RNA-seq experiment. You decide that you want to sift through the data for your own genes of interest. The first step is finding the GEO accession number corresponding to the dataset. To find it, you should navigate to the methods section and search (Ctrl-F) for "GSE". A identifier such as GSE<small>XXXXX</small>, where <small>X</small> represents a number, usually shows up in a statement such as: "the data have been deposited in GEO under accession number GSE<small>XXXXX</small>". If that doesn't work, try to Ctrl-F for "GEO". Once we have the accession number, we can now search GEO to find the dataset.
...
```{r echo = F}
include_graphics("images/GSE71165_bottom_of_page.png")
```

If we click on the **SRA Run Selector** link, we will see a table of identifers for each of the samples. What we are looking for are the SRA Run IDs, which start with the prefix "SRR". I've pointed out where to find them in the image below:

```{r echo = F, out.width = "100%"}
include_graphics("images/SRA_run_selector_GSE71165.png")
```

The SRA runs (e.g. SRR2121685) correspond to the actual sequencing files that we want to download in order to access the raw data for each sample. Sometimes, there will be multiple SRA runs for each sample, and they will be displayed as multiple rows in the table above, sharing a common GSM and SRX number. Usually this means that the lab had deposited multiple FASTQ files for one sample and did not bother to concatenate them together prior to deposition.

Instead of clicking on the SRA Run Selector, you can get more details about how each sample was prepared clicking on the GSM identifier in the **Samples** section from the first image (e.g., GSM1828772). This will take you to the sample description [page](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSM1828772). Scrolling down to the **Relations** section will show you the sample's SRA identifier, beginning with "SRX":

```{r echo = F}
include_graphics("images/GSM1828771_bottom_of_page.png")
```

Clicking on the SRA number (for example, SRX1114423 for HET_CD4_1) will bring you to yet another [page](https://www.ncbi.nlm.nih.gov/sra?term=SRX1114423) that provides information about the SRA runs for that sample (e.g. SRR2121685 for HET_CD4_1):

```{r echo = F}
include_graphics("images/SRX1114423_whole_page.png")
```
...
We observe that two fastq files have been extracted from SRR2121685.sra. This is because the original fastq file was produced from paired-end sequencing. `fastq-dump` has extracted the SRA file into two files, with suffix "_1" for paired-end read 1 and "_2" for  paired-end read 2. I typically use the settings provided above for `fastq-dump` as my default settings.
...
We are done downloading the FASTQ files! Now, we can start mapping the reads to a reference genome and perform downstream bulk RNA-sequencing analysis. If you want to see more, I analyze this data to find upregulated genes in the IMM_CD4_1 and IMM_CD4_2 samples compared to the HET_CD4_1 and HET_CD4_2 samples in this repo: https://github.com/erilu/dendritic-cell-bulk-rnaseq. I hope that this short tutorial has helped you learn how to use the SRA tools to download raw sequencing data. Thanks for reading!
